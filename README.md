...
# Zero-config mode (auto-detects current directory)
./target/release/tileserver-rs

# Auto-detect a specific directory or file
./target/release/tileserver-rs /path/to/tiles
./target/release/tileserver-rs /path/to/planet.pmtiles

# Override port in auto-detect mode
./target/release/tileserver-rs --port 3000 /path/to/tiles

# Explicit config still supported (takes precedence)
...
When started without `--config`, tileserver-rs auto-detects sources/styles in the target path (or current directory).
If `--config` is provided, auto-detection is disabled.
If `--config` points to a missing file, startup fails immediately with an error.
