...
We benchmarked tileserver-rs against martin and tileserver-gl using the same test data.

### PMTiles Performance (Florence, Italy)

| Server | Avg Req/sec | Avg Latency | Throughput |
|--------|-------------|-------------|------------|
| **tileserver-rs** | **1,420** | **78ms** | 93 MB/s |
| martin | 86 | 1,107ms | 9 MB/s |
| tileserver-gl | N/A | N/A | N/A |

> **tileserver-rs is 16x faster than martin for PMTiles serving.**

### MBTiles Performance (Zurich, Switzerland)

| Server | Avg Req/sec | Avg Latency | Throughput |
|--------|-------------|-------------|------------|
| **tileserver-rs** | 769 | 180ms | 93 MB/s |
| martin | **1,053** | **116ms** | 205 MB/s |
| tileserver-gl | 751 | 187ms | 90 MB/s |

> martin is faster for MBTiles due to optimized SQLite handling. tileserver-rs matches tileserver-gl performance.

*Note: martin and tileserver-gl were run in Docker containers; tileserver-rs ran natively.*
...
| **Static Images** | ✅ | ✅ | ❌ |
| **PMTiles Throughput** | **~93 MB/s** | ~50 MB/s | ~9 MB/s |
| **MBTiles Throughput** | ~93 MB/s | ~90 MB/s | **~205 MB/s** |

> **tileserver-rs** provides the best balance: excellent PMTiles performance, native MapLibre rendering for raster tiles, and static image generation - all in a single binary.
