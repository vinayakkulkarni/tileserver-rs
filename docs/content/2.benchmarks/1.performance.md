...
|--------|-------------|-------------|------------|
| **tileserver-rs** | **1,383** | **80ms** | 91 MB/s |
| martin | 70 | 1,379ms | 8 MB/s |
| tileserver-gl | N/A | N/A | N/A |

> **tileserver-rs is 20x faster than martin for PMTiles serving.**

### MBTiles Performance (Zurich, Switzerland)

| Server | Avg Req/sec | Avg Latency | Throughput |
|--------|-------------|-------------|------------|
| martin | **782** | **149ms** | 161 MB/s |
| **tileserver-rs** | 752 | 185ms | 92 MB/s |
| tileserver-gl | 736 | 192ms | 88 MB/s |

> All three servers perform similarly for MBTiles, with martin having a slight edge due to optimized SQLite handling.
...
| **Static Images** | ✅ | ✅ | ❌ |
| **PMTiles Req/sec** | **1,383** | N/A | 70 |
| **MBTiles Req/sec** | 752 | 736 | **782** |

> **tileserver-rs** provides the best balance: excellent PMTiles performance (20x faster than martin), native MapLibre rendering for raster tiles, and static image generation - all in a single binary.
