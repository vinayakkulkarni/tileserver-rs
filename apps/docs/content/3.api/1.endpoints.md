---
title: API Endpoints
description: REST API reference for Tileserver RS
---

Tileserver RS provides a REST API for accessing tiles and metadata.

## Interactive API Reference

```
GET /_openapi
```

Opens the [Scalar](https://scalar.com) API reference UI — an interactive browser for all endpoints with request/response schemas, code examples, and a built-in API client.

## OpenAPI Specification

```
GET /openapi.json
```

Returns the full OpenAPI 3.1 specification as JSON. Use this to generate client SDKs, import into Postman, or integrate with other API tools.

---

## Health Check

```
GET /health
```

Returns `OK` if the server is running.

**Response:** `200 OK`

## Ping (Runtime Metadata)

```
GET /ping
```

Returns runtime metadata about the running server instance. Useful for monitoring, debugging, and verifying configuration state.

**Response:**

```json
{
  "status": "ok",
  "config_hash": "a1b2c3d4e5f6...",
  "loaded_at_unix": 1708444800,
  "loaded_sources": 3,
  "loaded_styles": 2,
  "renderer_enabled": true,
  "prometheus_listener_active": false,
  "version": "2.8.1",
  "cache_enabled": true,
  "cache_entries": 128,
  "cache_bytes": 4194304,
  "render_enabled": true,
  "ogc_enabled": true,
  "compression_enabled": true,
  "compression_br_quality": 5,
  "compression_zstd_level": 3,
  "cors_origins": ["*"],
  "cache_dir": "/tmp/tileserver-rs"
}
```

| Field                        | Type     | Description                                            |
| ---------------------------- | -------- | ------------------------------------------------------ |
| `status`                     | string   | Always `"ok"`                                          |
| `config_hash`                | string   | SHA-256 hash of the current configuration              |
| `loaded_at_unix`             | integer  | Unix timestamp when config was last loaded             |
| `loaded_sources`             | integer  | Number of tile sources currently loaded                |
| `loaded_styles`              | integer  | Number of styles currently loaded                      |
| `renderer_enabled`           | boolean  | Whether native MapLibre rendering is available         |
| `prometheus_listener_active` | boolean  | Whether the Prometheus metrics listener is bound       |
| `version`                    | string   | Server version from `Cargo.toml`                       |
| `cache_enabled`              | boolean  | Whether the global tile cache is enabled               |
| `cache_entries`              | integer  | Current number of cached tiles                         |
| `cache_bytes`                | integer  | Approximate cache size in bytes                        |
| `render_enabled`             | boolean  | Whether static-image render routes are mounted         |
| `ogc_enabled`                | boolean  | Whether OGC API routes are mounted                     |
| `compression_enabled`        | boolean  | Whether `Accept-Encoding` re-encoding is active        |
| `compression_br_quality`     | integer  | Brotli quality level (0–11)                            |
| `compression_zstd_level`     | integer  | Zstandard compression level (1–22)                     |
| `cors_origins`               | string[] | Configured CORS allow-list                             |
| `cache_dir`                  | string   | Resolved scratch/state directory path                  |

::callout{type="warning"}
**`/ping` is public and unauthenticated.** `cors_origins` and `cache_dir` expose
your CORS allow-list and a filesystem path. If you treat these as sensitive,
gate `/ping` behind your reverse proxy in production.
::

## Hot Reload (Admin)

```
POST /__admin/reload
```

Reloads the server configuration without downtime. Only available on the admin server (configured via `admin_bind` in `[server]`).

**Query Parameters:**

| Name    | Type    | Description                                                      |
| ------- | ------- | ---------------------------------------------------------------- |
| `flush` | boolean | Force reload even if config hash is unchanged (default: `false`) |

**Response (success):**

```json
{
  "ok": true,
  "reloaded": true,
  "config_hash": "f6e5d4c3b2a1...",
  "loaded_at_unix": 1708444900,
  "loaded_sources": 4,
  "loaded_styles": 2,
  "renderer_enabled": true,
  "version": "2.8.1"
}
```

When `reloaded` is `false`, the config hash hasn't changed and no reload was performed (use `?flush=true` to force).

**Response (error):**

```json
{
  "ok": false,
  "error": "Failed to parse config: expected `=`, found `[` at line 5"
}
```

::alert{type="warning"}
On error, the server keeps the previous working configuration. No data is lost.
::

::alert{type="info"}
You can also reload by sending `SIGHUP` to the server process: `kill -HUP <pid>`. See the [Hot Reload Guide](/guides/hot-reload) for details.
::

## MCP OAuth Admin (Admin)

```
GET    /__admin/oauth/clients
DELETE /__admin/oauth/clients/{client_id}
GET    /__admin/oauth/sessions
DELETE /__admin/oauth/sessions/{token_id}
```

Admin-listener-only REST endpoints for inspecting and revoking MCP OAuth
clients (registered via Dynamic Client Registration) and their refresh-token
sessions. Available when the `mcp` feature is enabled with OAuth. Deletes are
idempotent — `{deleted: false}` is a normal success response. See the
[MCP Admin UI guide](/guides/mcp-admin-ui) for the web UI built on these.

---

## API Key Passthrough

All endpoints that return URLs support an optional `?key` query parameter. When provided, the key is appended to all URLs in the response, enabling API gateway/proxy scenarios where authentication needs to be forwarded through subsequent requests.

**Example:**

```
GET /styles/osm-bright/style.json?key=my_api_key
```

All URLs in the response will include `?key=my_api_key`:

```json
{
  "sources": {
    "openmaptiles": {
      "url": "http://localhost:8080/data/openmaptiles.json?key=my_api_key"
    }
  },
  "glyphs": "http://localhost:8080/fonts/{fontstack}/{range}.pbf?key=my_api_key",
  "sprite": "http://localhost:8080/styles/osm-bright/sprite?key=my_api_key"
}
```

**Supported endpoints:**

- `/data.json`
- `/data/{source}.json`
- `/index.json`
- `/styles.json`
- `/styles/{style}.json`
- `/styles/{style}/style.json`
- `/styles/{style}/wmts.xml`

---

## List All Sources

```
GET /data.json
```

Returns a list of all available tile sources with their TileJSON metadata.

**Query Parameters:**

| Name  | Type   | Description                                 |
| ----- | ------ | ------------------------------------------- |
| `key` | string | Optional API key to append to all tile URLs |

**Response:**

```json
[
  {
    "tilejson": "3.0.0",
    "name": "OpenMapTiles",
    "tiles": ["http://localhost:8080/data/openmaptiles/{z}/{x}/{y}.pbf"],
    "minzoom": 0,
    "maxzoom": 14,
    "bounds": [-180, -85.0511, 180, 85.0511],
    "center": [0, 0, 2]
  }
]
```

## Get Source TileJSON

```
GET /data/{source}.json
```

Returns TileJSON metadata for a specific source.

**Parameters:**

| Name     | Type   | Description |
| -------- | ------ | ----------- |
| `source` | string | Source ID   |

**Query Parameters:**

| Name  | Type   | Description                             |
| ----- | ------ | --------------------------------------- |
| `key` | string | Optional API key to append to tile URLs |

**Response:**

```json
{
  "tilejson": "3.0.0",
  "name": "OpenMapTiles",
  "tiles": ["http://localhost:8080/data/openmaptiles/{z}/{x}/{y}.pbf"],
  "vector_layers": [...],
  "minzoom": 0,
  "maxzoom": 14,
  "bounds": [-180, -85.0511, 180, 85.0511],
  "center": [0, 0, 2]
}
```

::alert{type="info"}
For sources using MapLibre Tiles (MLT) format, the TileJSON response includes an `"encoding": "mlt"` field and tile URLs use the `.mlt` extension.
::

## Get Tile

```
GET /data/{source}/{z}/{x}/{y}.{format}
```

Returns a single tile from a source. Works with all source types (PMTiles, MBTiles, PostgreSQL).

**Parameters:**

| Name     | Type    | Description                                      |
| -------- | ------- | ------------------------------------------------ |
| `source` | string  | Source ID (from config)                          |
| `z`      | integer | Zoom level (0-22)                                |
| `x`      | integer | Tile X coordinate                                |
| `y`      | integer | Tile Y coordinate                                |
| `format` | string  | Tile format (`pbf`, `mlt`, `png`, `jpg`, `webp`) |

**Examples:**

```bash
# PMTiles source
GET /data/openmaptiles/14/8192/5461.pbf

# MBTiles source
GET /data/terrain/10/512/340.pbf

# PostgreSQL table source
GET /data/buildings/12/2048/1365.pbf

# PostgreSQL function source
GET /data/custom_tiles/14/8580/5737.pbf
```

**Response Headers:**

| Header             | Description                                                                                                  |
| ------------------ | ------------------------------------------------------------------------------------------------------------ |
| `Content-Type`     | MIME type based on format (`application/x-protobuf` for PBF, `application/vnd.maplibre-vector-tile` for MLT) |
| `Content-Encoding` | `gzip` if tile is compressed                                                                                 |
| `Cache-Control`    | Caching headers                                                                                              |

**Response Codes:**

| Code  | Description                     |
| ----- | ------------------------------- |
| `200` | Tile found and returned         |
| `204` | Tile not found (empty response) |
| `400` | Invalid coordinates             |
| `404` | Source not found                |

## Composite Tiles (Multi-Source)

```
GET /data/{a+b+c}/{z}/{x}/{y}.pbf
GET /data/{composite-id}/{z}/{x}/{y}.pbf
GET /data/{a+b+c}.json
```

Merges the vector tiles of several sources into a single response. Two forms:

- **Ad-hoc** — join source ids with `+` (e.g. `/data/roads+water/10/512/340.pbf`).
- **Named** — define a `[[composites]]` entry in the config and reference it by id.

Members are fetched concurrently, gzip members are decompressed, MLT members are
transcoded to MVT, then layers are merged (colliding layer names append their
features). Missing members are skipped; if every member misses, an empty MVT is
returned with `200 OK`. Raster members are rejected with `400`.

**Composite TileJSON** (`/data/{id}.json`) unions member `vector_layers` (deduped
by layer id) and intersects the zoom range: `minzoom = max(members)`,
`maxzoom = min(members)`.

**Examples:**

```bash
# Ad-hoc composite of two sources
GET /data/protomaps+zurich/14/8716/5683.pbf

# Named composite (from [[composites]] id = "world-base")
GET /data/world-base/14/8716/5683.pbf

# Composite TileJSON
GET /data/protomaps+zurich.json
```

Config for a named composite:

```toml
[[composites]]
id = "world-base"
sources = ["protomaps", "zurich"]
```

See the [Composite Sources guide](/guides/composite-sources) for details.

## List Styles

```
GET /styles.json
```

Returns a list of all available map styles.

**Query Parameters:**

| Name  | Type   | Description                              |
| ----- | ------ | ---------------------------------------- |
| `key` | string | Optional API key to append to style URLs |

**Response:**

```json
[
  {
    "id": "protomaps-light",
    "name": "Protomaps Light",
    "url": "http://localhost:8080/styles/protomaps-light/style.json"
  }
]
```

## Get Style JSON

```
GET /styles/{style}/style.json
```

Returns the MapLibre/Mapbox GL style JSON for vector rendering.

**Query Parameters:**

| Name  | Type   | Description                                                       |
| ----- | ------ | ----------------------------------------------------------------- |
| `key` | string | Optional API key to append to all URLs (sources, glyphs, sprites) |

**Auto-generated styles:** when no `[[styles]]` entry matches `{style}` but a
**vector source** shares that id, a MapLibre v8 style is generated on the fly —
one `fill` + `line` + `circle` layer per source-layer, with deterministic colors
and zoom-interpolated widths. This lets any vector source be previewed without
authoring a style. Raster and unknown-format sources still return `404`. See the
[Style Auto-Generation guide](/guides/style-autogen).

## Get Raster TileJSON

```
GET /styles/{style}.json
```

Returns TileJSON metadata for raster tile rendering of a style.

**Query Parameters:**

| Name  | Type   | Description                                    |
| ----- | ------ | ---------------------------------------------- |
| `key` | string | Optional API key to append to raster tile URLs |

**Response:**

```json
{
  "tilejson": "3.0.0",
  "name": "Protomaps Light",
  "tiles": ["http://localhost:8080/styles/protomaps-light/{z}/{x}/{y}.png"],
  "minzoom": 0,
  "maxzoom": 22
}
```

## Get Raster Tile

```
GET /styles/{style}/{z}/{x}/{y}[@{scale}x].{format}
```

Returns a rendered raster tile from a vector style.

**Parameters:**

| Name     | Type    | Description                             |
| -------- | ------- | --------------------------------------- |
| `style`  | string  | Style ID                                |
| `z`      | integer | Zoom level (0-22)                       |
| `x`      | integer | Tile X coordinate                       |
| `y`      | integer | Tile Y coordinate                       |
| `scale`  | integer | Optional scale factor (1-3, default: 1) |
| `format` | string  | Output format (`png`, `jpeg`, `webp`)   |

**Examples:**

```
/styles/protomaps-light/14/8192/5461.png          # 512x512 PNG @ 1x
/styles/protomaps-light/14/8192/5461@2x.png       # 1024x1024 PNG @ 2x (retina)
/styles/protomaps-light/14/8192/5461.webp         # 512x512 WebP
```

### Explicit tile size

```
GET /styles/{style}/{tile_size}/{z}/{x}/{y}[@{scale}x].{format}
```

An optional `tile_size` path segment (`256` or `512` only) requests a specific
base tile edge before the scale factor is applied. Without it, tiles render at
the default 512px base:

```
/styles/protomaps-light/256/14/8192/5461.png      # 256x256 PNG @ 1x
/styles/protomaps-light/256/14/8192/5461@2x.png   # 512x512 PNG @ 2x
```

Any other value for `tile_size` is rejected with an error response.

**Performance:**

- Warm cache: ~100ms per tile
- Cold cache: ~700-800ms (includes tile fetching)

## Get Static Map Image

```
GET /styles/{style}/static/{type}/{width}x{height}[@{scale}x].{format}
```

Returns a static map image centered on a location or bounding box.

**Static Image Types:**

- **Center**: `{lon},{lat},{zoom}[@{bearing}[,{pitch}]]`
- **Bounding Box**: `{minx},{miny},{maxx},{maxy}`
- **Auto-fit**: `auto` (requires `?path=` or `?marker=` query params)

**Query Parameters:**

| Name      | Description                                                   |
| --------- | ------------------------------------------------------------- |
| `path`    | Path/polyline overlay (see format below)                      |
| `marker`  | Marker overlay (see format below)                             |
| `geojson` | URL-encoded GeoJSON Geometry, Feature, or FeatureCollection   |
| `padding` | Padding ratio for auto-fit (default: 0.1)                     |
| `maxzoom` | Maximum zoom level for auto-fit                               |
| `latlng`  | Parse coordinates as lat,lng instead of lng,lat (default: false) |

**Examples:**

```
# Center-based
/styles/protomaps-light/static/-122.4,37.8,12/800x600.png
/styles/protomaps-light/static/-122.4,37.8,12@45,60/800x600@2x.webp

# Bounding box
/styles/protomaps-light/static/-123,37,-122,38/1024x768.jpeg

# Auto-fit with path
/styles/protomaps-light/static/auto/800x600.png?path=path-5+f00(-122.4,37.8|-122.5,37.9|-122.4,38.0)

# With markers
/styles/protomaps-light/static/-122.4,37.8,12/800x600.png?marker=pin-s+f00(-122.4,37.8)
```

### Path Format

Paths are specified as: `path-{strokeWidth}+{strokeColor}({coordinates})`

- `strokeWidth`: Line width in pixels (default: 3)
- `strokeColor`: Hex color (3 or 6 digits, e.g., `f00` or `ff0000`)
- `coordinates`: Pipe-separated lon,lat pairs

Multiple paths can be separated by `~`.

**Example:**

```
path-5+ff0000(-122.4,37.8|-122.5,37.9|-122.4,38.0)
```

### Marker Format

Markers are specified as: `pin-{size}-{label}+{color}({lon},{lat})`

- `size`: `s` (small), `m` (medium), or `l` (large)
- `label`: Optional single character label
- `color`: Hex color

Multiple markers can be separated by `~`.

**Example:**

```
pin-s+f00(-122.4,37.8)~pin-m-A+00f(-122.5,37.9)
```

## Embeddable Map (iframe)

```
GET /embed/{style}
```

Returns a self-contained HTML page that renders the style with MapLibre GL JS — designed to be embedded via `<iframe>`. Works even when server-side rendering is disabled (`disable_render = true`).

**Query Parameters:**

| Name          | Default      | Description                                                                  |
| ------------- | ------------ | ---------------------------------------------------------------------------- |
| `center`      | —            | `lat,lng` (e.g. `37.8,-122.4`)                                               |
| `zoom`        | `2`          | Clamped to `0..=22`                                                          |
| `bearing`     | `0`          | Camera rotation in degrees                                                   |
| `pitch`       | `0`          | Camera tilt in degrees, clamped to `0..=85`                                  |
| `bounds`      | —            | `minLng,minLat,maxLng,maxLat`; overrides `center`/`zoom`                     |
| `markers`     | —            | Pipe-separated `lng,lat` pairs                                               |
| `controls`    | `navigation` | Comma-separated: `navigation`, `scale`, `fullscreen`                         |
| `hash`        | `false`      | Sync map position to the URL hash                                            |
| `interactive` | `true`       | `false` disables all pointer/keyboard input (preview mode)                   |
| `theme`       | auto         | `light` or `dark`; absent → follows `prefers-color-scheme`                   |

**Example:**

```html
<iframe
  src="https://tiles.example.com/embed/protomaps-light?center=37.8,-122.4&zoom=12"
  width="600" height="400" style="border: 0">
</iframe>
```

The page also exposes a bidirectional `postMessage` API (`flyTo`, `fitBounds`, `setFilter`, `setLayoutProperty` in; `ready`, `move`, `click` out). See the [Embeddable Maps guide](/guides/embed-iframe) for details.

## OpenGraph Image

```
GET /og/{style}
```

Returns a social-card static image (default **1200×630 PNG**, the OpenGraph standard size) rendered with the native renderer. Requires rendering to be enabled (same gating as raster tiles).

**Query Parameters:**

| Name     | Default | Description                                 |
| -------- | ------- | -------------------------------------------- |
| `center` | —       | `lat,lng`                                    |
| `zoom`   | `2`     | Zoom level                                   |
| `width`  | `1200`  | Image width in pixels (max 4096)             |
| `height` | `630`   | Image height in pixels (max 4096)            |
| `format` | `png`   | One of `png`, `jpg`, `jpeg`, `webp`          |

**Example:**

```
GET /og/protomaps-light?center=37.8,-122.4&zoom=10
```

See the [OpenGraph Images guide](/guides/og-images) for `<meta>` tag usage.

## Get Sprite

```
GET /styles/{style}/sprite[@{scale}x].{format}
```

Returns sprite image or metadata for a style.

**Parameters:**

| Name     | Type    | Description                               |
| -------- | ------- | ----------------------------------------- |
| `style`  | string  | Style ID                                  |
| `scale`  | integer | Optional scale factor (2x, 3x for retina) |
| `format` | string  | `png` for image, `json` for metadata      |

**Examples:**

```
/styles/protomaps-light/sprite.json       # Sprite metadata
/styles/protomaps-light/sprite.png        # Sprite image (1x)
/styles/protomaps-light/sprite@2x.json    # Retina metadata
/styles/protomaps-light/sprite@2x.png     # Retina image (2x)
```

## Get WMTS Capabilities

```
GET /styles/{style}/wmts.xml
```

Returns OGC WMTS 1.0.0 GetCapabilities document for use with GIS software (QGIS, ArcGIS).

**Query Parameters:**

| Name  | Type   | Description                                                              |
| ----- | ------ | ------------------------------------------------------------------------ |
| `key` | string | Optional API key to append to all tile URLs in the capabilities document |

---

## Font Endpoints

### List Fonts

```
GET /fonts.json
```

Returns a list of all available font families.

**Response:**

```json
["Noto Sans Regular", "Noto Sans Medium", "Open Sans Bold"]
```

### Get Font Glyphs

```
GET /fonts/{fontstack}/{range}.pbf
```

Returns font glyphs in PBF format for text rendering.

**Parameters:**

| Name        | Type   | Description                                 |
| ----------- | ------ | ------------------------------------------- |
| `fontstack` | string | Font name or comma-separated fallback stack |
| `range`     | string | Character range (e.g., `0-255`, `256-511`)  |

**Examples:**

```
/fonts/Noto Sans Regular/0-255.pbf
/fonts/Noto Sans Bold,Arial Unicode MS Regular/256-511.pbf
```

**Font Stack Fallback:**

When a font stack is requested (comma-separated), the server tries each font in order and returns the first available match. This enables fallback behavior for missing glyphs.

---

## Other Endpoints

### Combined Index

```
GET /index.json
```

Returns a combined array of all TileJSONs (both data sources and raster styles).

**Query Parameters:**

| Name  | Type   | Description                                 |
| ----- | ------ | ------------------------------------------- |
| `key` | string | Optional API key to append to all tile URLs |

**Response:**

```json
[
  {
    "tilejson": "3.0.0",
    "id": "openmaptiles",
    "name": "OpenMapTiles",
    "tiles": ["http://localhost:8080/data/openmaptiles/{z}/{x}/{y}.pbf"],
    ...
  },
  {
    "tilejson": "3.0.0",
    "name": "Protomaps Light",
    "tiles": ["http://localhost:8080/styles/protomaps-light/{z}/{x}/{y}.png"],
    ...
  }
]
```

### Get Tile as GeoJSON

```
GET /data/{source}/{z}/{x}/{y}.geojson
```

Returns a vector tile converted to GeoJSON format. Useful for debugging and inspection.

**Parameters:**

| Name     | Type    | Description                              |
| -------- | ------- | ---------------------------------------- |
| `source` | string  | Source ID (must be a vector tile source) |
| `z`      | integer | Zoom level                               |
| `x`      | integer | Tile X coordinate                        |
| `y`      | integer | Tile Y coordinate                        |

**Response:**

```json
{
  "type": "FeatureCollection",
  "features": [
    {
      "type": "Feature",
      "properties": {
        "name": "Example",
        "_layer": "places"
      },
      "geometry": {
        "type": "Point",
        "coordinates": [1205, 1540]
      }
    }
  ]
}
```

::alert{type="warning"}
GeoJSON coordinates are in tile coordinate space (0-4096), not geographic coordinates. This endpoint is primarily for debugging tile contents.
::

### Get Static File

```
GET /files/{filepath}
```

Serves static files from the configured files directory. Useful for serving GeoJSON overlays, custom icons, or other static assets.

**Parameters:**

| Name       | Type   | Description                                      |
| ---------- | ------ | ------------------------------------------------ |
| `filepath` | string | Path to the file relative to the files directory |

**Examples:**

```
/files/data.geojson
/files/icons/marker.png
/files/overlays/route.json
```

**Security:**

- Path traversal (`..`) is blocked
- Only files within the configured directory are accessible

**Response Headers:**

- `Content-Type`: Determined by file extension
- `Cache-Control`: `public, max-age=3600`

---
## Spatial Query Endpoints

These endpoints provide schema introspection, statistics, and feature queries for vector tile sources. Useful for building data explorers, search interfaces, and spatial analysis tools.

### Get Source Schema

```
GET /api/spatial/schema/{source}
```

Returns schema information for a source including vector layers, field names, and field types.

**Parameters:**

| Name     | Type   | Description |
| -------- | ------ | ----------- |
| `source` | string | Source ID   |

**Response:**

```json
{
  "source": "openmaptiles",
  "format": "pbf",
  "minzoom": 0,
  "maxzoom": 14,
  "bounds": [-180, -85.0511, 180, 85.0511],
  "layers": [
    {
      "id": "water",
      "minzoom": 0,
      "maxzoom": 14,
      "fields": [
        { "name": "class", "type": "String" },
        { "name": "intermittent", "type": "Number" }
      ]
    }
  ]
}
```

**Response Codes:**

| Code  | Description      |
| ----- | ---------------- |
| `200` | Schema returned  |
| `404` | Source not found |

### Get Source Stats

```
GET /api/spatial/stats/{source}
```

Returns statistics for a source including bounds, zoom range, layer count, and metadata.

**Parameters:**

| Name     | Type   | Description |
| -------- | ------ | ----------- |
| `source` | string | Source ID   |

**Response:**

```json
{
  "source": "openmaptiles",
  "format": "pbf",
  "minzoom": 0,
  "maxzoom": 14,
  "bounds": [-180, -85.0511, 180, 85.0511],
  "center": [0, 0, 2],
  "layer_count": 14,
  "name": "OpenMapTiles",
  "description": "Tileserver RS vector tiles",
  "attribution": "© OpenMapTiles © OpenStreetMap contributors"
}
```

**Response Codes:**

| Code  | Description      |
| ----- | ---------------- |
| `200` | Stats returned   |
| `404` | Source not found |

### Query Features

```
POST /api/spatial/query
```

Query features from a source by decoding vector tiles at the requested location. Returns features as JSON with layer name, geometry type, and properties.

**Request Body (JSON):**

| Field    | Type       | Required | Default | Description                              |
| -------- | ---------- | -------- | ------- | ---------------------------------------- |
| `source` | string     | Yes      | —       | Source ID to query                       |
| `bbox`   | `[f64; 4]` | No       | —       | Bounding box `[west, south, east, north]` |
| `zoom`   | integer    | No       | `14`    | Zoom level for tile resolution           |
| `layers` | `string[]` | No       | —       | Only return features from these layers   |
| `limit`  | integer    | No       | `100`   | Maximum features to return               |

**Example Request:**

```json
{
  "source": "openmaptiles",
  "bbox": [8.51, 47.36, 8.56, 47.40],
  "zoom": 14,
  "layers": ["building", "water"],
  "limit": 50
}
```

**Response:**

```json
{
  "source": "openmaptiles",
  "features": [
    {
      "layer": "building",
      "geometry_type": "Polygon",
      "properties": {
        "render_height": 20,
        "render_min_height": 0
      }
    }
  ],
  "total": 42,
  "truncated": false
}
```

**Response Codes:**

| Code  | Description                                    |
| ----- | ---------------------------------------------- |
| `200` | Features returned                              |
| `400` | Source is not a vector source (e.g., raster)   |
| `404` | Source not found or tile not found at location |

::alert{type="info"}
If no `bbox` is provided, the query uses the source's center coordinates. The `truncated` field indicates whether the result was limited by the `limit` parameter.
::

---

## File Upload Endpoints

These endpoints support the [drag-and-drop file visualization](/guides/file-drop) feature in the built-in map viewer. Uploaded files become temporary tile sources available until removed.

::alert{type="info"}
File uploads require the `upload_dir` option to be configured in `[server]`. See [Configuration](/getting-started/configuration) for details.
::

### Upload File

```
POST /api/upload
```

Upload a geospatial file for server-side processing. The file is streamed to disk chunk-by-chunk to prevent memory issues with large files. Once uploaded, the file is registered as a temporary tile source and immediately available for serving.

**Content-Type:** `multipart/form-data`

**Body:**

| Field  | Type | Description                  |
| ------ | ---- | ---------------------------- |
| `file` | file | The geospatial file to upload |

**Supported formats:**

| Format  | Extensions       | Description                                          |
| ------- | ---------------- | ---------------------------------------------------- |
| MBTiles | `.mbtiles`       | SQLite-based vector tile archive                     |
| SQLite  | `.sqlite`, `.db` | Treated as MBTiles                                   |
| COG     | `.tif`, `.tiff`  | Cloud Optimized GeoTIFF (requires `raster` feature)  |

**Response:**

```json
{
  "id": "a1b2c3d4-e5f6-...",
  "source_id": "upload-a1b2c3d4-e5f6-...",
  "file_name": "zurich.mbtiles",
  "format": "mbtiles",
  "tilejson_url": "http://localhost:8080/data/upload-a1b2c3d4-e5f6-....json"
}
```

**Response Codes:**

| Code  | Description                             |
| ----- | --------------------------------------- |
| `200` | File uploaded and source registered     |
| `400` | Unsupported file format or invalid file |
| `413` | File exceeds `upload_max_size_mb` limit |
| `500` | Failed to write file or register source |

::alert{type="warning"}
Uploaded sources are session-only — they are not persisted to config and will be lost on server restart.
::

### List Uploads

```
GET /api/upload
```

Returns a list of all currently uploaded file sources.

**Response:**

```json
[
  {
    "id": "a1b2c3d4-e5f6-...",
    "file_name": "zurich.mbtiles",
    "format": "mbtiles",
    "file_path": "/data/uploads/a1b2c3d4-e5f6-....mbtiles"
  }
]
```

### Delete Upload

```
DELETE /api/upload/{id}
```

Removes an uploaded source and deletes the file from disk. Accepts either the UUID or the full source ID (`upload-{uuid}`).

**Parameters:**

| Name | Type   | Description                   |
| ---- | ------ | ----------------------------- |
| `id` | string | Upload UUID or full source ID |

**Response Codes:**

| Code  | Description                 |
| ----- | --------------------------- |
| `204` | Upload deleted successfully |
| `404` | Upload not found            |
