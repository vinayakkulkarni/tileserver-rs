 ...
 *
 * Distinct from `admin-config.ts` (which mirrors the live
 * `GET /__admin/config` API response shape). These types describe the
 * hand-authored *catalog* of every config key the user could set, sourced
 * from `crates/tileserver-rs/src/config.rs` + the docs page; the actual
 * catalog data lives in `app/composables/admin/config-schema.ts`.
 *
 * Why separate from `admin-config.ts`:
 *   - `AdminConfigPayload` is what the server sends right now.
 *   - `ConfigSectionSchema[]` is what the server *could* accept.
 * Conflating both forces consumers to import both shapes for one or the
 * other concern.
 */
